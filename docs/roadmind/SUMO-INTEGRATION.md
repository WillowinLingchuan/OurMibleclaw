...
> 面向：**P2（接入）** 与 **P4（场景搭建）** ｜ 资料基于 SUMO 官方文档核对整理
...
|---|---|---|---|
| **A** | 安装 + 跑起来看到车 | P2 | 0.5 |
| **B** | 场景搭建（路网 + 车流 + 3 套预置场景） | P4 | 1.5 |
| **C** | TraCI 接入 + 决策适配层 | P2 | 1.5 |
| **D** | 回流指标 + 最小闭环脚本 | P2 | 1.5 |
| **E** | 与 Agent 打通 + 可复现验证 | P2 + P4 | 1 |
| | **合计** | | **约 6 人天（1 周）** |
...
| 人 | 做什么 |
|---|---|
| **P2 · 后端 / 架构** | 第一章（安装）、第三章（TraCI）、第四章（决策适配层）、第六章（闭环脚本） |
| **P4 · 业务 / 演示** | 第二章（路网 + 车流 + 3 套预置场景）、第五章指标口径、第七章验收 |
| **P3 · 算法** | 第六章的 `agent.decide()` —— 把 history 变成 next_rule 的策略 |
| **P1 · 队长** | 答辩话术：*"闭环在 SUMO 仿真器中验证，这是学术界通用工具，不是自制模拟器。"* |
